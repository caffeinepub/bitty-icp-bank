...
actor {
  ...
  // After distribution, send the winning remainder amount back to treasury (NOT the whole canister balance)
  func sendRemainderToTreasury(voteType : VoteType, winningAmount : Nat) : async () {
    ...
    let fee : Nat = 10000;
    if (winningAmount <= fee) return;
    let sendAmount = winningAmount - fee;
    try {
  ...
  // After distribution for custom proposals, send the winning remainder amount to specified address (NOT the whole canister balance)
  func sendRemainderToAddress(destination : Text, voteType : VoteType, winningAmount : Nat) : async () {
    if (winningAmount == 0) return;
    let ledgerId = switch (voteType) {
      case (#ICP) { "ryjl3-tyaaa-aaaaa-aaaba-cai" };
      case (#BITTYICP) { "qroj6-lyaaa-aaaam-qeqta-cai" };
